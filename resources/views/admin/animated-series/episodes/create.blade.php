<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Серия для «{{ $animatedSeries->title }}» · {{ $season->label() }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Заполни карточку серии. Файл не загружается — начни вводить имя уже лежащего в
                        <span class="font-mono break-all">{{ $directory }}</span>
                    </p>

                    <form method="POST" action="{{ route('admin.animated-series.episodes.store', [$animatedSeries, $season]) }}" class="mt-6 space-y-4">
                        @csrf

                        <div>
                            <x-input-label for="number" value="Номер серии" />
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title')" required placeholder="Пилот" />
                        ...
                        <div
                            x-data="{
                                url: {{ \Illuminate\Support\Js::from(route('admin.animated-series.episodes.search')) }},
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>Добавить серию</x-primary-button>
                            <a href="{{ route('admin.animated-series.edit', $animatedSeries) }}" class="text-sm text-gray-500 underline">Назад</a>
                        </div>
                    </form>
                </div>
            </div>
