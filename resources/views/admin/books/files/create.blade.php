            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Например «PDF» или «Том 1». Файл не загружается — начни вводить имя уже лежащего в
                        <span class="font-mono break-all">{{ $directory }}</span>
                    </p>

                    <form method="POST" action="{{ route('admin.books.files.store', $book) }}" class="mt-6 space-y-4">
                        @csrf
                        ...
                        <div
                            x-data="{
                                url: {{ \Illuminate\Support\Js::from(route('admin.books.files.search')) }},
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('admin.books.edit', $book) }}" class="text-sm text-gray-500 underline">Назад</a>
                        </div>
                    </form>
                </div>
            </div>
