<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.games') }}" class="text-sm text-gray-500 underline">К списку</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('admin.games.update', $game) }}" class="mt-6 space-y-4" enctype="multipart/form-data">
                        @csrf
                        @method('put')

                        @include('admin.games.partials.fields', [
                            'title' => $game->getTranslation('title', \App\Enums\Locale::Ru->value, false),
                            'year' => $game->year,
                            'description' => $game->getTranslation('description', \App\Enums\Locale::Ru->value, false),
                        ])
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <a href="{{ route('admin.games.files.create', $game) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white">
                            Добавить файл
                        </a>
                    </div>

                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        ISO, exe, zip и другие инсталляторы из папки games. Загрузки нет.
                    </p>

                    <div class="mt-6 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($game->files as $file)
                            <div class="flex flex-col gap-3 py-4 first:pt-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between">
                                ...
                                <form method="POST" action="{{ route('admin.games.files.destroy', [$game, $file]) }}" onsubmit="return confirm('Убрать файл? На диске он останется.')">
                                    @csrf
                                    @method('delete')
