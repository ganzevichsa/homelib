    ...
    >
        @if ($series->hasPoster())
            <div class="pointer-events-none absolute inset-x-0 top-0 hidden h-[36rem] overflow-hidden sm:block">
                <img src="{{ route('library.series.poster', $series) }}" alt="" class="h-full w-full scale-110 object-cover opacity-25 blur-2xl">
        ...
        <div class="relative mx-auto max-w-6xl px-4 pb-24 pt-3 sm:px-8 sm:pt-4">
            <a href="{{ route('library.show', \App\Enums\MediaType::Series) }}" class="inline-flex min-h-11 items-center text-sm text-white/40 transition hover:text-white">← Сериалы</a>
            ...
            <div class="mt-4 flex items-start gap-4 sm:mt-8 sm:gap-6 lg:grid lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-8">
                ...
                <div class="min-w-0">
                    <h1 class="text-2xl font-extrabold tracking-tight sm:text-4xl lg:text-6xl">{{ $series->title }}</h1>

                    @if ($series->original_title)
                        <p class="mt-2 text-sm text-white/50 sm:mt-3 sm:text-lg">{{ $series->original_title }}</p>
                    @endif

                    <div class="mt-3 flex flex-wrap items-center gap-1.5 text-xs text-white/55 sm:mt-5 sm:gap-2 sm:text-sm">
                        @if ($series->year)
                            <span class="rounded-full bg-white/10 px-2.5 py-1 sm:px-3">{{ $series->year }}</span>
                        @endif
                    ...
                    @if ($series->description)
                        <p class="mt-4 hidden max-w-3xl text-base leading-relaxed text-white/70 sm:mt-6 sm:block">{{ $series->description }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="-mx-4 mt-6 overflow-hidden bg-black sm:mx-0 sm:mt-12 sm:rounded-3xl sm:ring-1 sm:ring-white/10">
                ...
                @else
                    <div class="flex aspect-video items-center justify-center px-6 text-center text-white/40">
                        К этому сериалу ещё не добавлены серии.
                    </div>
                @endif
            </div>

            @if ($series->description)
                <p class="mt-6 text-sm leading-relaxed text-white/70 sm:hidden">{{ $series->description }}</p>
            @endif
